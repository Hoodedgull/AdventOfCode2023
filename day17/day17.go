...
func main() {
	...
	for len(beams) > 0 {
		...
		for _, opt := range options {
			...
			s := []rune(opt.path)
			last3 := string(s[max(len(s)-3, 0):])

			stepSize := 0
			if strings.HasSuffix(last3, strings.Repeat(opt.dir, 3)) {
				stepSize = 3
			} else if strings.HasSuffix(last3, strings.Repeat(opt.dir, 2)) {
				stepSize = 2
			} else if strings.HasSuffix(last3, strings.Repeat(opt.dir, 1)) {
				stepSize = 1
			}
			foundAGoodCase := false
			for step := stepSize; step <= 3; step++ {
...
func getOptions(beam Beam, grid [][]int) []Beam {
	beams := []Beam{}
	if beam.dir != "w" && !strings.HasSuffix(beam.path, "eee") {
		x := beam.x + 1
		y := beam.y
	...
	if beam.dir != "e" && !strings.HasSuffix(beam.path, "www") {
		x := beam.x - 1
		y := beam.y
	...
	if beam.dir != "s" && !strings.HasSuffix(beam.path, "nnn") {
		x := beam.x
		y := beam.y - 1
	...
	if beam.dir != "n" && !strings.HasSuffix(beam.path, "sss") {
		x := beam.x
		y := beam.y + 1
